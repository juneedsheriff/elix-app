/**
 * Applies clinic PSE isolation migration (045).
 *   npm run db:apply-clinic-pse
 *   npm run db:apply-clinic-pse -- --password=YOUR_DB_PASSWORD
 */
...
const migrationFile = '045_clinic_pse.sql';
...
console.log('Applying clinic PSE migration (045)…');
...
Add one of these to .env.local, then run: npm run db:apply-clinic-pse

  SUPABASE_DB_PASSWORD=your-database-password
  SUPABASE_ACCESS_TOKEN=sbp_...   (https://supabase.com/dashboard/account/tokens)

Or paste supabase/migrations/045_clinic_pse.sql into Supabase Dashboard → SQL Editor → Run.
`);
  process.exit(1);
}
...
console.log('Clinic PSE migration applied successfully.');
