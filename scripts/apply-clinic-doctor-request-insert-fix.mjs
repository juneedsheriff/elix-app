/**
 * Fix clinic_doctor_requests insert RLS (migration 052).
 *   npm run db:apply-clinic-doctor-request-insert-fix
 */
...
const migrationFile = '052_clinic_doctor_requests_insert_fix.sql';
...
console.log('Applying clinic doctor request insert fix (052)…');
...
if (connectionStrings.length === 0) {
  console.error(
    'Missing SUPABASE_DB_PASSWORD. Run supabase/migrations/052_clinic_doctor_requests_insert_fix.sql in SQL Editor.'
  );
  process.exit(1);
}
...
console.log('Clinic doctor request insert fix applied successfully.');
