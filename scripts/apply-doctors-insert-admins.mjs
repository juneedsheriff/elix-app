/**
 * Applies doctors insert policy column (migration 027).
 *   npm run db:apply-doctors-insert-admins
 *   npm run db:apply-doctors-insert-admins -- --password=YOUR_DB_PASSWORD
 */
...
const migrationFile = '028_doctors_insert_admins.sql';
...
async function probeApplied() {
  // RLS policy migration is idempotent (drop + create). Always apply via npm script.
  return false;
}
...
console.log('Applying doctors insert policy migration (028)…');
...
Add one of these to .env.local, then run: npm run db:apply-doctors-insert-admins

  SUPABASE_DB_PASSWORD=your-database-password
  SUPABASE_ACCESS_TOKEN=sbp_...   (https://supabase.com/dashboard/account/tokens)

Or paste supabase/migrations/028_doctors_insert_admins.sql into Supabase Dashboard → SQL Editor → Run.
`);
  process.exit(1);
}
...
console.log('Doctors insert policy migration applied successfully.');
