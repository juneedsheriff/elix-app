/**
 * Clinic PSE delete for clinic-scoped opinion requests (migration 071).
 *   npm run db:apply-clinic-pse-delete-requests
 *   npm run db:apply-clinic-pse-delete-requests -- --password=YOUR_DB_PASSWORD
 */
...
const migrationFile = '071_clinic_pse_delete_opinion_requests.sql';
...
console.log('Applying clinic PSE delete opinion requests (071)…');
...
if (connectionStrings.length === 0) {
  console.error(`
Missing SUPABASE_DB_PASSWORD. Run supabase/apply-clinic-pse-delete-requests-in-sql-editor.sql in SQL Editor.
`);
  process.exit(1);
}
...
console.log('Clinic PSE can now delete clinic-scoped opinion requests.');
