/**
 * Applies patient case details columns (migration 041).
 *   npm run db:apply-patient-case-details
 */
...
const migrationFile = '041_patient_case_details.sql';
...
async function probeApplied() {
  ...
  const probe = await supabase.from('opinion_requests').select('patient_case_details').limit(1);
  return !probe.error;
}
...
if (await probeApplied()) {
  console.log('Patient case details columns already exist.');
  process.exit(0);
}

console.log('Applying patient case details migration (041)…');
...
Add one of these to .env.local, then run: npm run db:apply-patient-case-details

  SUPABASE_DB_PASSWORD=your-database-password
  SUPABASE_ACCESS_TOKEN=sbp_...   (https://supabase.com/dashboard/account/tokens)

Or paste supabase/migrations/041_patient_case_details.sql into Supabase Dashboard → SQL Editor → Run.
`);
  process.exit(1);
}
...
if (!(await probeApplied())) {
  console.error('Migration ran but patient_case_details column may still be missing.');
  process.exit(1);
}

console.log('Patient case details migration applied successfully.');
