/**
 * Applies patient detach request records policy (migration 043).
 *   npm run db:apply-patient-detach-request-records
 */
...
const migrationFile = '043_patient_detach_request_records.sql';
const policyName = 'opinion_request_records_delete_own_unverified';
...
async function queryWithPg(connectionStrings, sql) {
  let lastError;
...
async function applyWithPg(connectionStrings, sql) {
  await queryWithPg(connectionStrings, sql);
...
async function probeApplied(connectionStrings, accessToken) {
  const probeSql = `
    select 1
    from pg_policies
    where schemaname = 'public'
      and tablename = 'opinion_request_records'
  ...
  if (connectionStrings.length > 0) {
    try {
      const result = await queryWithPg(connectionStrings, probeSql);
  ...
  try {
    const response = await fetch(`https://api.supabase.com/v1/projects/${projectRef}/database/query`, {
      ...
      body: JSON.stringify({ query: probeSql })
    });
...
if (await probeApplied(connectionStrings, accessToken)) {
  console.log('Patient detach request records policy already exists.');
  process.exit(0);
}

console.log('Applying patient detach request records migration (043)…');

if (connectionStrings.length === 0 && !accessToken) {
  console.error(`
Missing database credentials for DDL migration.

Add one of these to .env.local, then run: npm run db:apply-patient-detach-request-records

  SUPABASE_DB_PASSWORD=your-database-password
  SUPABASE_ACCESS_TOKEN=sbp_...   (https://supabase.com/dashboard/account/tokens)

Or paste supabase/migrations/043_patient_detach_request_records.sql into Supabase Dashboard → SQL Editor → Run.
`);
  process.exit(1);
}
...
if (!(await probeApplied(connectionStrings, accessToken))) {
  console.error('Migration ran but opinion_request_records delete policy may still be missing.');
  process.exit(1);
}

console.log('Patient detach request records migration applied successfully.');
