/**
 * Applies patient medical history columns (migration 059).
 *   npm run db:apply-patient-medical-history
 */
...
const migrationFile = '059_patient_medical_history.sql';
...
async function probeApplied() {
  ...
  const probe = await supabase
    .from('patients')
    .select('family_history, social_history, surgical_history, medical_history')
    .limit(1);
  return !probe.error;
}
...
if (await probeApplied()) {
  console.log('Patient medical history columns already exist.');
  process.exit(0);
}

console.log('Applying patient medical history migration (059)…');
...
Add one of these to .env.local, then run: npm run db:apply-patient-medical-history

  SUPABASE_DB_PASSWORD=your-database-password
  SUPABASE_ACCESS_TOKEN=sbp_...   (https://supabase.com/dashboard/account/tokens)

Or paste supabase/migrations/059_patient_medical_history.sql into Supabase Dashboard → SQL Editor → Run.
`);
  process.exit(1);
}
...
if (!(await probeApplied())) {
  console.error('Migration ran but medical history columns may still be missing.');
  process.exit(1);
}

console.log('Patient medical history migration applied successfully.');
