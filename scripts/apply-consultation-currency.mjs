/**
 * Applies consultation currency (migration 036).
 *   npm run db:apply-consultation-currency
 */
...
const migrationFile = '036_doctor_consultation_currency.sql';
...
async function probeApplied() {
  ...
  const probe = await supabase.from('doctors').select('consultation_currency').limit(1);
  return !probe.error;
}
...
if (await probeApplied()) {
  console.log('Consultation currency migration already applied.');
  process.exit(0);
}

console.log('Applying consultation currency migration (036)…');
...
if (!(await probeApplied())) {
  console.error('Migration ran but doctors.consultation_currency may still be missing.');
  process.exit(1);
}

console.log('Consultation currency migration applied successfully.');
