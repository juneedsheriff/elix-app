/**
 * Applies migration 077 (update_own_doctor_profile RPC).
 *   node scripts/apply-update-own-doctor-profile.mjs
 */
...
const migrationFile = '077_update_own_doctor_profile.sql';
...
if (connectionStrings.length === 0) {
  console.error(`
Missing SUPABASE_DB_PASSWORD (or DATABASE_URL). Set it in .env.local, or paste
supabase/migrations/${migrationFile} into the Supabase SQL Editor.

Note: \`supabase db push\` needs \`supabase link\` first (no project is linked in this repo).
`);
  process.exit(1);
}
...
try {
  ...
  console.log('Doctor own-profile RPC applied successfully.');
} catch (error) {
