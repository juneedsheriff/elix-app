/**
 * Consultation summary prescription / lab order file columns (migration 079).
 *   npm run db:apply-consultation-prescription-lab-order-files
 */
...
const migrationFile = '079_consultation_prescription_lab_order_files.sql';
...
function buildDbUrlsFromPassword() {
  const password =
    ...
    readPasswordFromArgs() ||
    process.env.PROMPTED_DB_PASSWORD?.trim();
  if (!password) return [];
  const ref =
...
console.log('Applying consultation prescription / lab order file columns (079)…');
...
if (connectionStrings.length === 0) {
  console.error(`
Missing SUPABASE_DB_PASSWORD. Run supabase/migrations/${migrationFile} in SQL Editor.
`);
  process.exit(1);
}
...
console.log('Doctors can store prescription and lab order file paths on consultation summaries.');
