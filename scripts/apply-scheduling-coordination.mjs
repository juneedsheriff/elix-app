/**
 * Applies scheduling coordination columns (migration 023).
 *   npm run db:apply-scheduling-coordination
 *   npm run db:apply-scheduling-coordination -- --password=YOUR_DB_PASSWORD
 */
...
const migrationFile = '023_scheduling_coordination.sql';
...
async function probeApplied() {
  ...
  const probe = await supabase.from('opinion_requests').select('payment_link').limit(1);
  return !probe.error;
}
...
if (await probeApplied()) {
  console.log('Records verification column (records_verified_at) already exists.');
  process.exit(0);
}

console.log('Applying records verification migration (021)…');
...
Add one of these to .env.local, then run: npm run db:apply-records-verified

  SUPABASE_DB_PASSWORD=your-database-password
  SUPABASE_ACCESS_TOKEN=sbp_...   (https://supabase.com/dashboard/account/tokens)

Or paste supabase/migrations/021_records_verified.sql into Supabase Dashboard → SQL Editor → Run.
`);
  process.exit(1);
}
...
if (!(await probeApplied())) {
  console.error('Migration ran but records_verified_at column may still be missing.');
  process.exit(1);
}

console.log('Records verification migration applied successfully.');
