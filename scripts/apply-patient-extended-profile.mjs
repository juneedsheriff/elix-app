/**
 * Applies patient extended profile columns (migration 031).
 *   npm run db:apply-patient-extended-profile
 */
...
const migrationFile = '031_patient_extended_profile.sql';
...
async function probeApplied() {
  ...
  const probe = await supabase.from('patients').select('address, height_cm, weight_kg').limit(1);
  return !probe.error;
}
...
if (await probeApplied()) {
  console.log('Patient extended profile columns already exist.');
  process.exit(0);
}

console.log('Applying patient extended profile migration (031)…');
...
Add one of these to .env.local, then run: npm run db:apply-patient-extended-profile

  SUPABASE_DB_PASSWORD=your-database-password
  SUPABASE_ACCESS_TOKEN=sbp_...   (https://supabase.com/dashboard/account/tokens)

Or paste supabase/migrations/031_patient_extended_profile.sql into Supabase Dashboard → SQL Editor → Run.
`);
  process.exit(1);
}
...
if (!(await probeApplied())) {
  console.error('Migration ran but address/height_cm/weight_kg columns may still be missing.');
  process.exit(1);
}

console.log('Patient extended profile migration applied successfully.');
