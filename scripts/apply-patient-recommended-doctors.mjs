/**
 * Lets patients read doctors recommended on their opinion requests (migration 056).
 *   npm run db:apply-patient-recommended-doctors
 */
...
const migrationFile = '056_patient_recommended_doctors_select.sql';
...
console.log('Applying patient recommended doctors SELECT policy (056)…');
...
if (connectionStrings.length === 0) {
  console.error(
    'Missing SUPABASE_DB_PASSWORD. Run supabase/migrations/056_patient_recommended_doctors_select.sql in SQL Editor.'
  );
  process.exit(1);
}
...
console.log('Patient recommended doctors policy applied successfully.');
