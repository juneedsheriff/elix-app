/**
 * Applies admin delete policy for opinion requests (migration 024).
 *   npm run db:apply-admin-delete-requests
 *   npm run db:apply-admin-delete-requests -- --password=YOUR_DB_PASSWORD
 */
...
const migrationFile = '024_admin_delete_opinion_requests.sql';
...
console.log('Applying admin delete opinion requests policy (024)…');
...
Add one of these to .env.local, then run: npm run db:apply-admin-delete-requests

  SUPABASE_DB_PASSWORD=your-database-password
  SUPABASE_ACCESS_TOKEN=sbp_...   (https://supabase.com/dashboard/account/tokens)

Or paste supabase/apply-admin-delete-requests-in-sql-editor.sql into Supabase Dashboard → SQL Editor → Run.
`);
  process.exit(1);
}

const migration024 = readFileSync(join(root, 'supabase/migrations', migrationFile), 'utf8');
const migration025 = readFileSync(join(root, 'supabase/migrations', '025_admin_delete_opinion_requests_rpc.sql'), 'utf8');
const sql = `${migration024}\n${migration025}`;
...
console.log('Admin delete opinion requests policy and RPC functions applied successfully.');
