/**
 * Applies auth email registration check (migration 032).
 *   npm run db:apply-auth-email-check
 *   npm run db:apply-auth-email-check -- --password=YOUR_DB_PASSWORD
 */
...
const migrationFile = '032_auth_email_registered_check.sql';
...
async function probeApplied() {
  ...
  const probe = await supabase.rpc('is_auth_email_registered', { p_email: 'probe@example.com' });
  return !probe.error;
}
...
if (await probeApplied()) {
  console.log('Auth email check function (is_auth_email_registered) already exists.');
  process.exit(0);
}

console.log('Applying auth email check migration (032)…');
...
Add one of these to .env.local, then run: npm run db:apply-auth-email-check

  SUPABASE_DB_PASSWORD=your-database-password
  SUPABASE_ACCESS_TOKEN=sbp_...   (https://supabase.com/dashboard/account/tokens)

Or paste supabase/apply-auth-email-check-in-sql-editor.sql into Supabase Dashboard → SQL Editor → Run.
`);
  process.exit(1);
}
...
if (!(await probeApplied())) {
  console.error('Migration ran but is_auth_email_registered may still be missing.');
  process.exit(1);
}

console.log('Auth email check migration applied successfully.');
