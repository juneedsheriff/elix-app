/**
 * Applies consultation summary PDF column (migration 027).
 *   npm run db:apply-consultation-summary-pdf
 *   npm run db:apply-consultation-summary-pdf -- --password=YOUR_DB_PASSWORD
 */
...
const migrationFile = '027_consultation_summary_pdf.sql';
...
async function probeApplied() {
  ...
  const probe = await supabase.from('consultation_summaries').select('pdf_storage_path').limit(1);
  return !probe.error;
}
...
if (await probeApplied()) {
  console.log('consultation summary PDF column (pdf_storage_path) already exists.');
  process.exit(0);
}

console.log('Applying consultation summary PDF migration (027)…');
...
Add one of these to .env.local, then run: npm run db:apply-consultation-summary-pdf

  SUPABASE_DB_PASSWORD=your-database-password
  SUPABASE_ACCESS_TOKEN=sbp_...   (https://supabase.com/dashboard/account/tokens)

Or paste supabase/migrations/027_consultation_summary_pdf.sql into Supabase Dashboard → SQL Editor → Run.
`);
  process.exit(1);
}
...
if (!(await probeApplied())) {
  console.error('Migration ran but pdf_storage_path column may still be missing.');
  process.exit(1);
}

console.log('consultation summary PDF migration applied successfully.');
