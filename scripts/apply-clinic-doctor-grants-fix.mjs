/**
 * Fix clinic PSE visibility of approved platform doctors (migration 050).
 *   npm run db:apply-clinic-doctor-grants-fix
 */
...
const migrationFile = '050_fix_clinic_doctor_grants_visibility.sql';
...
console.log('Applying clinic doctor grants visibility fix (050)…');
...
if (connectionStrings.length === 0) {
  console.error(
    'Missing SUPABASE_DB_PASSWORD. Run supabase/migrations/050_fix_clinic_doctor_grants_visibility.sql in SQL Editor.'
  );
  process.exit(1);
}
...
console.log('Clinic doctor grants visibility fix applied successfully.');
