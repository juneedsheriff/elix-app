/**
 * Applies proceed-without-records columns (migration 042).
 *   npm run db:apply-proceeded-without-records
 */
...
const migrationFile = '042_proceeded_without_records.sql';
...
async function probeApplied() {
  ...
  const probe = await supabase.from('opinion_requests').select('patient_proceeded_without_records_at').limit(1);
  return !probe.error;
}
...
if (await probeApplied()) {
  console.log('Proceed-without-records columns already exist.');
  process.exit(0);
}

console.log('Applying proceed-without-records migration (042)…');
...
Add one of these to .env.local, then run: npm run db:apply-proceeded-without-records

  SUPABASE_DB_PASSWORD=your-database-password
  SUPABASE_ACCESS_TOKEN=sbp_...   (https://supabase.com/dashboard/account/tokens)

Or paste supabase/migrations/042_proceeded_without_records.sql into Supabase Dashboard → SQL Editor → Run.
`);
  process.exit(1);
}
...
if (!(await probeApplied())) {
  console.error('Migration ran but patient_proceeded_without_records_at column may still be missing.');
  process.exit(1);
}

console.log('Proceed-without-records migration applied successfully.');
