/**
 * Applies consultation invoice columns (migration 037).
 *   npm run db:apply-consultation-invoice
 */
...
const migrationFile = '037_consultation_invoice.sql';
...
async function probeApplied() {
  ...
  const probe = await supabase.from('opinion_requests').select('invoice_pdf_storage_path').limit(1);
  return !probe.error;
}
...
if (await probeApplied()) {
  console.log('Consultation invoice columns already exist on opinion_requests.');
  process.exit(0);
}

console.log('Applying consultation invoice migration (037)…');
...
Add one of these to .env.local, then run: npm run db:apply-consultation-invoice

  SUPABASE_DB_PASSWORD=your-database-password
  SUPABASE_ACCESS_TOKEN=sbp_...   (https://supabase.com/dashboard/account/tokens)

Or paste supabase/migrations/037_consultation_invoice.sql into Supabase Dashboard → SQL Editor → Run.
`);
  process.exit(1);
}
...
if (!(await probeApplied())) {
  console.error('Migration ran but invoice columns may still be missing.');
  process.exit(1);
}

console.log('Consultation invoice migration applied successfully.');
