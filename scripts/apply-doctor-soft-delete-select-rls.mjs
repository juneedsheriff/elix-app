/**
 * Soft-delete SELECT RLS fix for doctors (migration 073).
 *   npm run db:apply-doctor-soft-delete-select-rls
 *   npm run db:apply-doctor-soft-delete-select-rls -- --password=YOUR_DB_PASSWORD
 */
...
const migrationFile = '073_doctor_soft_delete_select_rls.sql';
...
console.log('Applying doctor soft-delete SELECT RLS fix (073)…');
...
if (connectionStrings.length === 0) {
  console.error(`
Missing SUPABASE_DB_PASSWORD. Run supabase/migrations/073_doctor_soft_delete_select_rls.sql in SQL Editor.
`);
  process.exit(1);
}
...
console.log('Clinic PSE and admins can soft-delete doctors without SELECT RLS conflicts.');
