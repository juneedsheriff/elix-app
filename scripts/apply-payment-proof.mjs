/**
 * Applies payment proof columns (migration 026).
 *   npm run db:apply-payment-proof
 */
...
const migrationFile = '026_payment_proof.sql';
...
async function probeApplied() {
  ...
  const probe = await supabase.from('opinion_requests').select('payment_proof_submitted_at').limit(1);
  return !probe.error;
}
...
if (await probeApplied()) {
  console.log('Payment proof columns already exist.');
  process.exit(0);
}

console.log('Applying payment proof migration (026)…');
...
if (connectionStrings.length === 0 && !accessToken) {
  console.error(`
Missing database credentials. Run: npm run db:apply-payment-proof

Or paste supabase/migrations/026_payment_proof.sql into Supabase SQL Editor.
`);
  process.exit(1);
}
...
if (!(await probeApplied())) {
  console.error('Migration ran but payment_proof_submitted_at may still be missing.');
  process.exit(1);
}

console.log('Payment proof migration applied successfully.');
