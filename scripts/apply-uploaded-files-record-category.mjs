/**
 * Applies uploaded_files record category columns (migration 044).
 *   npm run db:apply-uploaded-files-record-category
 */
...
const migrationFile = '044_uploaded_files_record_category.sql';
...
if (await probeApplied(connectionStrings, accessToken)) {
  console.log('uploaded_files.record_category column already exists.');
  process.exit(0);
}

console.log('Applying uploaded files record category migration (044)…');

if (connectionStrings.length === 0 && !accessToken) {
  console.error(`
Missing database credentials for DDL migration.

Add one of these to .env.local, then run: npm run db:apply-uploaded-files-record-category

  SUPABASE_DB_PASSWORD=your-database-password
  SUPABASE_ACCESS_TOKEN=sbp_...   (https://supabase.com/dashboard/account/tokens)

Or paste supabase/migrations/044_uploaded_files_record_category.sql into Supabase Dashboard → SQL Editor → Run.
`);
  process.exit(1);
}
...
if (!(await probeApplied(connectionStrings, accessToken))) {
  console.error('Migration ran but uploaded_files.record_category may still be missing.');
  process.exit(1);
}

console.log('Uploaded files record category migration applied successfully.');
