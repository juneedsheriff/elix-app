/**
 * Soft-delete SELECT RLS fix for patients (migration 072).
 *   npm run db:apply-patient-soft-delete-select-rls
 *   npm run db:apply-patient-soft-delete-select-rls -- --password=YOUR_DB_PASSWORD
 */
...
const migrationFile = '072_patient_soft_delete_select_rls.sql';
...
console.log('Applying patient soft-delete SELECT RLS fix (072)…');
...
if (connectionStrings.length === 0) {
  console.error(`
Missing SUPABASE_DB_PASSWORD. Run supabase/apply-patient-soft-delete-select-rls-in-sql-editor.sql in SQL Editor.
`);
  process.exit(1);
}
...
console.log('Clinic PSE and admins can soft-delete patients without SELECT RLS conflicts.');
