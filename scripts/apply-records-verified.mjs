/**
 * Applies records verification column (migration 021).
 *   npm run db:apply-records-verified
 *   npm run db:apply-records-verified -- --password=YOUR_DB_PASSWORD
 */
...
const migrationFile = '021_records_verified.sql';
...
async function probeApplied() {
  ...
  const probe = await supabase.from('opinion_requests').select('records_verified_at').limit(1);
  return !probe.error;
}
