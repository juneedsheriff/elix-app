...
__name(isLocalDevOrigin, "isLocalDevOrigin");
...
async function verifyAdmin(request, env) {
  ...
  if (!userId)
    return null;
  const { data, error } = await userClient(env, token).from("admins").select("id").eq("auth_user_id", userId).eq("is_active", true).maybeSingle();
  if (error || !data)
    return null;
  return userId;
}
...
async function findUserByEmail(email, admin) {
  const { data, error } = await admin.auth.admin.listUsers({ page: 1, perPage: 1e3 });
  if (error || !data.users)
    return null;
...
async function enableLogin(profile, role, password, env) {
  const email = profile.email?.trim().toLowerCase();
  if (!email)
    return { error: "Profile email is required to enable login." };
  if (password.length < 6)
    return { error: "Password must be at least 6 characters." };
  const table = role === "doctor" ? "doctors" : "patients";
  const admin = serviceClient(env);
  ...
  const patch = { auth_user_id: authUserId, login_disabled: false };
  if (role === "doctor") {
    patch.email = email;
    if (profile.mobile_no ?? profile.phone)
      patch.phone = profile.mobile_no ?? profile.phone;
  }
...
__name(enableLogin, "enableLogin");
async function disableLogin(profile, role, env) {
  const table = role === "doctor" ? "doctors" : "patients";
  const admin = serviceClient(env);
  if (profile.auth_user_id) {
    const { error: error2 } = await admin.auth.admin.updateUserById(profile.auth_user_id, {
...
__name(disableLogin, "disableLogin");
async function setPassword(profile, password, env) {
  if (!profile.auth_user_id)
    return { error: "No auth account linked. Enable login first." };
  if (password.length < 6)
    return { error: "Password must be at least 6 characters." };
...
async function getAccountStatus(role, profileId, env) {
...
__name(getAccountStatus, "getAccountStatus");
var src_default = {
