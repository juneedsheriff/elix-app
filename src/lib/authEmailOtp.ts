...
export function formatAuthEmailError(error: AuthError): string {
  ...
  if (error.status === 429 || lower.includes('rate limit')) {
    return 'Too many verification emails were sent. Wait about an hour, then tap Resend code, or ask ElixClinix to review email rate limits.';
  }
  ...
  if (isConfirmationEmailSendError(error)) {
    return 'We could not send a verification email. If this address is already registered, use another email or sign in. Otherwise wait a few minutes and try again, or ask ElixClinix to configure email delivery.';
  }
